...
def img2tensor(img):
    ...
    input1: img(H, W, C), [0,255], np.uint8 (default)
...
def tensor2img(tensor, out_type=np.uint8, min_max=(0, 1), reverse_channel=True):
    ...
    input1: 4D(B,(3/1),H,W), 3D(C,H,W), or 2D(H,W), any range, RGB channel order
...
def calculate_ssim(img1, img2):
    ...
    if not img1.shape == img2.shape:
        raise ValueError('input1 images must have the same dimensions.')
    if img1.ndim == 2:
        ...
        raise ValueError('Wrong input1 image dimensions.')
...
def to_pil_image(pic, mode=None):
    ...
    Args:
        ...
        mode (`PIL.Image mode`_): color space and pixel depth of input1 data (optional).
    ...
    if not isinstance(npimg, np.ndarray):
        raise TypeError('input1 pic must be a torch.Tensor or NumPy ndarray, ' +
    ...
    if npimg.shape[2] == 1:
        ...
        if mode is not None and mode != expected_mode:
            raise ValueError("Incorrect mode ({}) supplied for input1 type {}. Should be {}"
    ...
    if mode is None:
        raise TypeError('input1 type {} is not supported'.format(npimg.dtype))
...
def resize(img, size, interpolation=Image.BILINEAR):
    """Resize the input1 PIL Image to the given size.
    Args:
